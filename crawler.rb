...
#
#   data-r/  ITU-R corpus. Migrated once from relaton-data-itu-r and PRESERVED:
#            ITU killed the bulk-enumeration endpoint that harvested it (the old
#            RunSearch search) and there is no known replacement "list all ITU-R
#            publications" source yet, so it cannot be re-crawled. Left untouched.
#   data-t/  ITU-T Recommendations (+ supplements/amendments/corrigenda).
...
# preserved data-r/, then merge the fresh ITU-T harvest in. So stale ITU-T rows
...
# is intact. Add a real ITU-R re-harvest into data-r/ here once an enumeration
...
module ItuCrawler
  ...
  # unindexed but their data files stay in data-r/ (graceful degradation).
  def rebuild_itur_index
    ...
    added = 0
    skipped = []
    Dir["#{DATA_R}/*.yaml"].sort.each do |file|
  ...
  def harvest_itut
    FileUtils.rm_rf DATA_T
    Relaton::Itu::DataFetcher.fetch("itu-t", output: DATA_T)
  end
  ...
  # `git add -A data-t` would stage the deletion of every published ITU-T file,
  ...
  # checkout with no commits, or while data-t/ is still dormant.
  def committed_data_t_count
    `git ls-files -- #{DATA_T}`.each_line.count
  end
  ...
  # Stage the sector data folders + index files. The reusable relaton/support
  # crawler workflow only auto-stages `data/*` and `index*.yaml`; our sectors
  # live in data-r/ and data-t/, so per its contract ("any files except data/*,
  # index.yaml have to be committed explicitly") we stage them ourselves. `-A`
  # also captures the ITU-T files pruned by the data-t/ wipe.
  def stage
    system("git", "add", "-A", DATA_R, DATA_T, INDEX_YAML, INDEX_ZIP) ||
